...
func (suite *EventDispatcherTestSuite) TestEventDispacher_Register() {
	...
	assert.Equal(suite.T(), &suite.handler2, suite.eventDispatcher.handlers[suite.event2.GetName()][0])
}

func (suite *EventDispatcherTestSuite) TestEventDispathcer_Register_WithSameHandler() {
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
...
func (suite *EventDispatcherTestSuite) TestEventDispatcher_Remote() {
	err := suite.eventDispatcher.Register(suite.event.GetName(), &suite.handler)
	suite.Nil(err)
	suite.Equal(1, len(suite.eventDispatcher.handlers[suite.event.GetName()]))
